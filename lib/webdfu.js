...
;(() => {
  ...
  dfu.Device.prototype.logDebug = (msg) => {
    console.log("[WebDFU Debug]", msg)
  }

  dfu.Device.prototype.logInfo = (msg) => {
    console.log("[WebDFU Info]", msg)
  }

  dfu.Device.prototype.logWarning = (msg) => {
    console.log("[WebDFU Warning]", msg)
  }

  dfu.Device.prototype.logError = (msg) => {
    console.log("[WebDFU Error]", msg)
  }

  dfu.Device.prototype.logProgress = (done, total) => {
    if (typeof total === "undefined") {
      console.log("[WebDFU Progress]", done)
    } else {
      console.log("[WebDFU Progress]", done + "/" + total)
    }
  }
  ...
  dfu.parseDeviceDescriptor = (data) => ({
  ...
  dfu.parseConfigurationDescriptor = (data) => {
  ...
  dfu.parseInterfaceDescriptor = (data) => ({
    bLength: data.getUint8(0),
    bDescriptorType: data.getUint8(1),
  ...
  // -----------------------------
  // DFUSe (ST) vendor extensions
  // -----------------------------
  // Send a DFUSe command in block 0 and poll to IDLE
  dfu.Device.prototype.dfuseCommand = async function (cmdBytes /* ArrayBuffer(5) */) {
    // Always be in a sane state first
    try { await this.abortToIdle() } catch (_) {}
    await this.download(cmdBytes, 0);                      // block 0 is command channel
    const st = await this.poll_until_idle(dfu.dfuDNLOAD_IDLE);
    if (st.status !== dfu.STATUS_OK) {
      throw `DFUSe command failed state=${st.state} status=${st.status}`;
    }
  }

  // 0x21: Set Address Pointer (LE32)
  dfu.Device.prototype.dfuseSetAddress = async function (addr) {
    const buf = new ArrayBuffer(5);
    const v = new DataView(buf);
    v.setUint8(0, 0x21);
    v.setUint32(1, addr, true);
    await this.dfuseCommand(buf);
  }

  // 0x41: Erase page at address (LE32)
  ...
  // For DFUSe, pass first_block=2 (block 0 is commands)
  dfu.Device.prototype.do_download = async function (xfer_size, data, manifestationTolerant, first_block = 0) {
    let transaction = first_block
    const blocks = []
    let bytes_read = first_block
